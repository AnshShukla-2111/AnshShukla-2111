...
const FriendProfile = () => {
    ...
    const [friendPosts, setfriendPost] = useState([]);
    ...
    let getFriendData = async () => {
      let res = await axios.get(
        `http://localhost:9000/user/friend/${friendId}`,
        {
          headers: {
    ...
    const handleFollow = async () => {
      let res = await axios.put(
        `http://localhost:9000/user/followUnfollow/${friendId}`,
        {},
        {
          headers: {
            Authorization: userSlice.token,
      ...
      <div className="box flex gap-3 justify-center items-center mt-10 text-xl">
        {friend?.followers?.includes(userSlice?.user?._id) ? (
          <button
            ...
            className="px-5 py-2 rounded-md bg-[#022702] text-white hover:bg-[#008000]"
          >
            UnFollow
          </button>
        ) : (
          <button
            onClick={handleFollow}
            className="px-5 cursor-pointer py-2 rounded-md bg-[#480210] text-white hover:bg-[crimson] "
          >
            Follow
          </button>
        )}

        <button className="px-5 cursor-pointer py-2 rounded-md bg-[#000028] text-white hover:bg-[blue]">
          Chat
        </button>
      </div>

      {friendPosts.length > 0 ? (
        <div className="flex w-[400px] m-auto mt-[60px] flex-col gap-2">
          {friendPosts.map((ele, i) => {
            return (
              <PostCard getAllPosts={getFriendData} key={ele._id} ele={ele} />
            );
          })}
        </div>
      ) : (
        <h1 className="text-center md:text-5xl sm:text-3xl text-xl mt-[60px]">
          No Post to show
        </h1>
      )}
    </div>
